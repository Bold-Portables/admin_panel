...
const InventoryServiceRequests = (props: MyComponentProps) => {
  ...
  const getInventoryServicesData = async () => {
    setLoading(true);
    await authAxios()
      .get(`/user-service/find-by-inventory/${inventory._id}?page=${currentPage}&limit=${itemsPerPage}`)
      .then(
        (response) => {
          setLoading(false);
                ...
                <div className="nk-block-between">
                  <div className="nk-block-head-content">
                    <h3 className="nk-block-title page-title">
                      Service Requests for <span className="text-primary">{inventory.qrId}</span>
                    </h3>
